<?php
class ControllerExtensionModulePiwik extends Controller {
	protected $error = array();
	
	public function index() {   
		$this->load->language('extension/module/piwik');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('extension/module/piwik', 'user_token=' . $this->session->data['user_token'], 'SSL'));
		}
 		...
 		if (isset($this->error['site_id'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/piwik', 'user_token=' . $this->session->data['user_token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$data['action'] = $this->url->link('extension/module/piwik', 'user_token=' . $this->session->data['user_token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('extension/module/piwik', $data));	
	}
	
	// Validate the user inputs in the POST data.
	private function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/piwik')) {
